...
/**
 * Create a unified hardened SMTP transporter.
 * Uses explicit SSL config for both Gmail and production reliability.
 */
...
async function sendPayslipEmail(payslip) {
  if (!process.env.EMAIL_USER || !process.env.EMAIL_PASS) {
    throw new Error('Email credentials not configured. Add EMAIL_USER and EMAIL_PASS in Vercel Dashboard.');
  }
  ...
  try {
    ...
    throw new Error(
      `SMTP Authentication Failed: ${verifyErr.message}. ` +
      `Ensure EMAIL_PASS is a 16-character Gmail App Password, not your regular password.`
    );
  }
  ...
  const mailOptions = {
    from: `"${payslip.companyName}" <${(process.env.EMAIL_FROM || process.env.EMAIL_USER || '').trim()}>`,
