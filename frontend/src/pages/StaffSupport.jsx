...
function ContextView({ type, data, loading }) {
  ...
  if (type === 'Leave Request Issue') {
    if (!data || data.length === 0) return <EmptyContext text="No leave records found for this staff." />
    return (
  ...
  if (type === 'Payslip / Salary Issue') {
    if (!data || data.length === 0) return <EmptyContext text="No payslips found for this staff." />
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {data.map((ps, i) => (
          <div key={i} style={{
  ...
  return <EmptyContext text="Review the staff's message below and respond accordingly." />
}
    ...
    <div style={{ padding: '0 0 48px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          <div style={{ width: 46, height: 46, borderRadius: 13, background: '#ede9fe', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            <Headphones size={22} color="#7c3aed" />
          </div>
          <div>
            <h1 style={{ margin: 0, fontSize: 22, fontWeight: 800, color: 'var(--text)', letterSpacing: '-0.02em' }}>Staff Support</h1>
            <div style={{ fontSize: 13, color: 'var(--text-muted)', marginTop: 2 }}>Review and respond to staff requests</div>
          </div>
        </div>
        <button
        ...
        {loading ? (
          ...
          <div style={{ padding: 64, textAlign: 'center', color: 'var(--text-muted)' }}>
            ...
            <div style={{ fontSize: 13, marginTop: 4 }}>
              {filter === 'All' ? 'Staff requests will appear here.' : `No requests with ${filter} status.`}
            </div>
          </div>
        ) : (
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 700 }}>
              <thead>
                <tr style={{ background: 'var(--bg)', borderBottom: '1px solid var(--border)' }}>
                  {['Staff', 'Request', 'Date', 'Status', 'Actions'].map(h => (
                    <th key={h} style={{
              ...
              <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--border)', flexShrink: 0 }}>
                ...
                {viewModal.message && (
                  <div style={{ marginTop: 14, padding: '10px 14px', background: 'var(--bg)', borderRadius: 10, borderLeft: '3px solid #7c3aed' }}>
                    <div style={{ fontSize: 10, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 4 }}>Staff Message</div>
                    ...
                    <div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 3 }}>
                      {actionModal.action === 'approve'
                        ? actionModal.requestType === 'Attendance / Punch Issue'
                          ? "This will reset the staff's punch-out so they can re-punch."
                          : 'Staff will be notified of the approval.'
                        : 'Staff will be notified of the rejection.'}
                    </div>
                  </div>
                </div>
              </div>
